...
After staring at them for a few minutes, you work out what they mean:
```
...
but would still be facing east, so would move east if the following action were F.)
...
Actually almost all of the actions indicate how to move a waypoint which is relative to the ship's position:
```
...
Your puzzle input (starting numbers) is **13,16,0,12,15,1**.

### Part 2

Determine the 30000000th number spoken. For example, given the same starting numbers:
```
...
And you can conclude that in your ticket, class is 12, row is 11, and seat is 13.
...
###
```
(note in the above starting state the other dimensions - z=-1 and z=1 - all cubes are inactive `.`)
...
Add a fourth dimension to the above!
...
N.B. Basically the expression `1 + 2 * 3 + 4 * 5 + 6` is evaluated like this...
```(1 + 2) * (3 + 4) * (5 + 6)``` (33)
instead of like...
```1 + (2 * 3) + (4 * 5) + 6``` (231)
...
You are sent a list of the rules that valid messages should obey,
and a list of received messages (your puzzle input).
...
For example, the rule `2: 1 3 | 3 1` means that to match rule 2, the text being checked must match rule 1 followed by rule 3,
